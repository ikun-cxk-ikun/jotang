...
**Softmax 函数**：
- **定义**：Softmax 函数通常用于多分类问题的输出层，它将输入值映射为概率分布。Softmax函数本质上是一种归一化函数，可以将一组任意的实数值转化为在[0,1]之间的概率值。
...
### 9.卷积神经网络结构
**结构**：卷积神经网络主要由若干个输入层、卷积层、激活层、池化层、全连接层等部分组成。
**卷积层**做卷积运算，**激活层**用**激活函数**进行**非线性处理**，**池化层**用于降低特征图的维度（**缩小高、长方向上的空间的运算**），减少计算量，同时保持重要特征，**全连接层**，位于CNN的末端，用于将卷积层和池化层提取的特征转换为最终的输出结果，控制输出类别，起到分类器的作用。


**池化层的特征**
- **没有要学习的参数**：池化层和卷积层不同，没有要学习的参数。池化只是从目标区域中取最大值（或平均值），所以不存在要学习的参数。
- **通道数不变化**：经过池化运算，输入数据和输出数据的通道数不会发生变化。
- **对微小的位置变化具有鲁棒性（健壮）**：输入数据发生微小偏差时，池化仍会返回相同的结果
  在卷积神经网络（CNN）中，计算卷积层和池化层的输出大小是非常重要的。这里我们将详细介绍如何计算这些层的输出大小。
...
**例子:**
假设输入图像大小为 \( 32 \times 32 \), 使用 \( 3 \times 3 \) 的卷积核，步长为 \( 1 \), 填充为 \( 1 \).

\[ N = \frac{32 - 3 + 2 \times 1}{1} + 1 = 32 \]

所以，输出大小仍然是 \( 32 \times 32 \).
...
**例子:**
假设输入图像大小为 \( 32 \times 32 \), 使用\( 2 \times 2 \)  的最大池化,步长为\( 2  \)。
好的,让我们来看一个池化层的例子。

带入公式计算:
\[ N = \frac{32 - 2}{2} + 1 = 16 \]

所以经过 2x2 最大池化,步长为 2 之后,输出特征图的大小将变为 16x16。

### 10.归一化和标准化：

1. **归一化**：
   - 常用方法：Min-Max归一化，即将数据线性缩放到[0, 1]或[-1, 1]区间内。
   - 公式：\(x_{\text{new}} = \frac{x - x_{\text{min}}}{x_{\text{max}} - x_{\text{min}}}\)（对于[0, 1]区间）；或 \(x_{\text{new}} = \frac{2(x - x_{\text{min}})}{x_{\text{max}} - x_{\text{min}}} - 1\)（对于[-1, 1]区间）。

2. **标准化**：
   - 常用方法：Z-Score标准化，即将数据转换为均值为0、标准差为1的分布。
   - 公式：\(x_{\text{new}} = \frac{x - \mu}{\sigma}\)，其中\(\mu\)是样本数据的均值，\(\sigma\)是样本数据的标准差。

3. **归一化**：
   - 适用于输入数据分布较为均匀的情况。
   - 常用于图像处理、某些机器学习算法（如支持向量机、神经网络等）的预处理中。

4. **标准化**：
   - 适用于输入数据分布不均匀、存在较大离群值的情况。
   - 广泛用于机器学习算法的预处理中，特别是那些基于梯度的优化算法（如梯度下降、随机梯度下降等）。
